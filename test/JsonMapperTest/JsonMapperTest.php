...
use JsonMapper\Cache\Impl\ArrayCacheProvider;
use JsonMapper\Converter\Impl\CachedConverterRegistry;
...
use JsonMapper\Metadata\Impl\CachedMetadataProvider;
...
class JsonMapperTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        list($metadataCache, $converterCache) = [new ArrayCacheProvider(), new ArrayCacheProvider()];
        $converterRegistry = new DefaultConverterRegistry();
        $converterRegistry->register(new CustomTypeConverter());
        $converterRegistry = new CachedConverterRegistry($converterRegistry, $converterCache);
        $metadataProvider = new CachedMetadataProvider(new DefaultMetadataProvider(), $metadataCache);
        $this->jm = new JsonMapper();
        $this->jm->setConverterRegistry($converterRegistry);
        $this->jm->setMetadataProvider($metadataProvider);
    }
    ...
    public function testFromJson_primitiveProperties_nullPropertiesOmitted()
    {
        $input = ['boolProperty' => true, 'stringProperty' => 'abc'];

        $output = $this->jm->fromJson($input, PrimitivePropertiesClass::CLASS_PATH);

        $this->assertEquals(true, $output->getBoolProperty());
        $this->assertNull($output->getIntProperty());
        $this->assertEquals('abc', $output->getStringProperty());
        $this->assertNull($output->getDoubleProperty());
    }
    ...
    public function testFromJson_emptyArrayOfObjects_objectProperties()
    {
        $input = ['primitives' => []];
        ...
        $this->assertCount(0, $output->getPrimitives());
    }
